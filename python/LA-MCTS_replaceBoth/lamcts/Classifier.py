...
class Classifier():
    ...
    def propose_rand_samples_gaussian(self, nums_samples, path, lb, ub):
            
        weight = np.exp(self.fX/4).reshape((-1,1))
        denominator = weight.sum()
        center = (self.X * weight).sum(axis = 0)/denominator

        final_cands = np.random.normal(loc = center, scale = 1./len(path), 
            size = (nums_samples,self.dims))
        final_cands = final_cands[(final_cands <= ub).all(axis=1) & (final_cands >= lb).all(axis=1)]
    ...
    def propose_samples_bo( self, nums_samples = 10, path = None, lb = None, ub = None, samples = None):
        ...
        X    = self.propose_rand_samples_gaussian(nums_rand_samples, path, lb, ub)
    ...
    def learn_clusters(self):
        ...
        plabel = []
        mean_fX = self.fX.mean()
        for i in range(self.X.shape[0]):
            plabel.append(0 if self.fX[i] >= mean_fX else 1)
